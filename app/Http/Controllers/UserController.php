...
class UserController extends Controller
{
  ...
  public function request(Request $request)
  {
    ...
    if ($usr != null) {
      ...
      if ($request->uri == "") {
        Log::info('Valor de $request->uri', ['uri' => $request->uri]);
        if ($usr->empresa->configNota == null) {
            Log::info('Configuração de nota não encontrada para a empresa', ['empresa_id' => $usr->empresa->id]);
            session()->flash('flash_warning', "Por favor configure o emitente");
            return redirect('/configNF');
        } else {
            Log::info('Configuração de nota encontrada', ['empresa_id' => $usr->empresa->id]);
            $rotaInicial = env('ROTA_INICIAL', '/graficos');
            if ($usr->rota_acesso == "") {
                Log::info('Rota de acesso não definida, usando ROTA_INICIAL', ['rota_inicial' => env('ROTA_INICIAL')]);
                return redirect($rotaInicial);
            } else {
                Log::info('Redirecionando para rota de acesso do usuário', ['rota_acesso' => $usr->rota_acesso]);
                return redirect($usr->rota_acesso);
            }
        }
    } else {
        Log::info('Redirecionando para $request->uri', ['uri' => $request->uri]);
